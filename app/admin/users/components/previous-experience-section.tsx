...
export function PreviousExperienceSection({ experience }: PreviousExperienceSectionProps) {
    if (experience.length === 0) {
        return (
            <div className="space-y-4">
                <h3 className="text-lg font-medium">Service History</h3>
                <p className="text-sm text-muted-foreground italic">No service history recorded.</p>
            </div>
        )
    }
        ...
        <div className="space-y-6">
            ...
            <div className="relative border-l-2 border-muted ml-3 space-y-8 pb-2">
                {experience.map((group, groupIndex) => (
                    <div key={`${group.community}-${groupIndex}`} className="relative pl-6">
                        {/* Timeline dot */}
