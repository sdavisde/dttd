...
async function getFolderSize(supabase: SupabaseClient, bucketName: string, folderPath: string = ''): Promise<number> {
  const { data: items, error } = await supabase.storage.from(bucketName).list(folderPath)

  if (error) {
    throw new Error(`Failed to list items in ${bucketName}/${folderPath}: ${error.message}`)
  }
