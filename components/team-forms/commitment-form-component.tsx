...
export function CommitmentFormComponent({
  ...
  const onSubmit = async (data: CommitmentFormValues) => {
    ...
    if (isErr(result)) {
      toastError('Unable to save commitment form. Please try again.', {
        error: result.error,
      })
      ...
      <CardContent className="space-y-6">
        <div className="bg-muted/30 p-6 rounded-lg border space-y-4">
          <Typography variant="p">
            I, <strong>{userName}</strong>, affirm that I am a Lay / Clergy
            person who is consistent in my walk with Christ, faithful in my
            local church, supportive of my pastor and an active participant,
            endeavoring to be an Authentic Christian Leader in my personal
            Piety, Study and Action and will strive to present the highest moral
            standard before the team and candidates; have attended a Tres Dias
            or other approved similar weekend; have been selected by the rector
            and approved by the Tres Dias Secretariat, agree to serve as{' '}
            <strong>{userRole}</strong> on <strong>{weekendTitle}</strong>.
          </Typography>
          <Typography variant="p" className="font-medium italic">
            Please read and check each blank affirming that you are in
            compliance.
          </Typography>
        </div>
