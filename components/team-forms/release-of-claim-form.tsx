...
const releaseOfClaimSchema = z
  ...
  .refine(
    (data) => {
      if (
        data.has_special_needs === 'yes' &&
        isEmpty(data.special_needs_description)
      ) {
        return false
      }
      return true
...
export function ReleaseOfClaimForm({
  ...
  const hasSpecialNeeds = useWatch({
      ...
      <CardContent className="space-y-6">
        <div className="bg-muted/30 p-6 rounded-lg border space-y-4 text-sm">
          <Typography variant="p">
            I understand that Dusty Trails Tres Dias (DTTD) does not own
            Tanglewood Christian Camp where my Tres Dias Weekend will take
            place, and has limited control over the facility.
          </Typography>
          <Typography variant="p">
            I therefore agree to <strong>RELEASE AND INDEMNIFY DTTD</strong> to
            the same extent that I{' '}
            <strong>RELEASE AND INDEMNIFY Tanglewood Christian Camp</strong> for
            any and all injuries which I may receive while participating in the
            Weekend.
          </Typography>
          <Typography variant="p">
            I further and expressly{' '}
            <strong>RELEASE, INDEMNIFY AND HOLD HARMLESS DTTD</strong>, its
            officers and directors, and any volunteers participating in/on the
            weekend from any and all claims for personal injury, death, or loss
            or destruction of property.
          </Typography>
          <Typography variant="p">
            I further represent and warrant that I have disclosed in writing to
            DTTD, on this form, each and every medical condition or issue I am
            aware of having which does or could require medications, special
            diets, sleeping accommodations, restrooms or other special
            considerations or facilities.
          </Typography>
          <Typography variant="p">
            I am not aware of any physical, mental, or emotional limitations I
            have which would make it difficult for me to participate in normal
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="space-y-3">
                    <FormLabel>
                      Do you have any special needs (bottom bunk, mobility
                      issues, special diet or medications)?
                    </FormLabel>
                    <FormControl>
                      <RadioGroup
