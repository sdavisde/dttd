...
export function FileContextMenu({ anchorEl, open, onClose, item, bucket, currentPath }: FileContextMenuProps) {
  ...
  const handleDownload = () => {
    if (!item.isFolder) {
      window.open(`/api/files/download?bucket=${bucket}&path=${fullPath}`, '_blank')
    }
    onClose()
  }
      ...
      {open && (
        <>
          ...
          >
            ...
            <button
              className="flex w-full items-center gap-2 rounded-sm px-2 py-1.5 text-sm hover:bg-accent hover:text-accent-foreground"
              onClick={() => { setRenameOpen(true); onClose() }}
            >
            ...
            <button
              ...
              onClick={() => { setDeleteOpen(true); onClose() }}
            >
        ...
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              {item.isFolder ? <FolderIcon className="w-5 h-5" /> : <FileIcon className="w-5 h-5" />}
          ...
          <div className="space-y-4">
            ...
            <Alert>
              <AlertDescription>
                ...
                {item.isFolder && ' All files within this folder will also be deleted.'}
              </AlertDescription>
            </Alert>
          </div>
